...
if sys.platform == "win32":
    ...
    RdpWidget, TEM_FRDP, ERRO_FRDP = None, False, "gtk-frdp não existe no Windows"
else:
    try:
        from rdp import RdpWidget, TEM_FRDP, ERRO_FRDP
    except Exception as e:
        RdpWidget, TEM_FRDP, ERRO_FRDP = None, False, str(e)
    _rdp_win, TEM_RDP_WINDOWS = None, False
# a classe de verdade (AbaRdpWindows) so pode ser MONTADA depois que
# AbaBase e CapturaTeclado existirem — ver perto da construcao da AbaSsh,
# mais abaixo, onde _rdp_win.construir(...) é chamado.
AbaRdpWindows = None
...
# quebrar.
# ---------------------------------------------------------------------------
if sys.platform == "win32" and TEM_RDP_WINDOWS:
    try:
...
if sys.platform == "win32":
    ...
    # visual mais simples (sem cores ANSI ainda, sem biblioteca de
    # snippets) mas com login automatico e deteccao de troca de host key
    # preservados. Ver ssh_windows.py para o porque.
    try:
        import ssh_windows as _ssh_win
        AbaSsh = _ssh_win.construir(
            AbaBase, CapturaTeclado, add_class=add_class)
        TEM_VTE = _ssh_win.TEM_TERMINAL_WINDOWS
        ERRO_VTE = _ssh_win.ERRO_TERMINAL_WINDOWS
    except Exception as e:
        AbaSsh, TEM_VTE = None, False
        ERRO_VTE = "módulo ssh_windows.py indisponível: %s" % e
else:
    try:
        import ssh as _ssh
...
class Janela(Gtk.Window):
    ...
    def abrir(self, cx, tipo="vnc", focar=True):
        ...
        if tipo == "rdp" and sys.platform == "win32":
    ...
    def _editar_ini(self, _b=None):
        ...
        # "C:\Users\..." nao e uma file: URI valida (faltam a barra e a
        # conversao das barras invertidas), e o Gtk.show_uri_on_window
        # falha em silencio. os.startfile e o caminho nativo — ele usa a
        # associacao do Explorer.
        #
        # E .ini FREQUENTEMENTE nao tem programa associado (o Windows nao
        # associa por padrao), caso em que o startfile levanta OSError.
        # Por isso o notepad entra como reserva explicita: e o que o
        # operador esperaria de qualquer jeito.
        if sys.platform == "win32":
            try:
                os.startfile(self.caminho)          # noqa: S606
                return
            except Exception as e:
                sys.stderr.write("startfile falhou (%s); tentando notepad\n"
                                 % e)
            for prog in ("notepad.exe", "notepad"):
                try:
                    subprocess.Popen([prog, self.caminho])
                    return
                except Exception:
                    continue
            self.avisar("Não consegui abrir o INI",
                        "Abra manualmente:\n%s" % self.caminho)
            return
...
def main():
    ...
    # LOGIN ACEITO: a partir daqui so a janela GTK importa. O console do
    # lancador (acessos.cmd) fica escondido e vira um icone de bandeja —
    # ver bandeja_windows.py para o porque (reduz o risco de um Ctrl+C
    # sem querer matar o processo, e tira a janela do console da barra de
    # tarefas). Sem efeito fora do Windows.
    if sys.platform == "win32":
        try:
            import bandeja_windows
            bandeja_windows.iniciar(caminho_icone=icone)
        except Exception:
            pass
    ...
    if sys.platform == "win32":
        try:
            bandeja_windows.definir_ao_sair(j.close)
        except Exception:
            pass
