...
# --------------------------------------------------------------- shim
#
# PORTABILIDADE WINDOWS: unico ponto deste arquivo que muda por plataforma
# é o nome/carregador da biblioteca — o shim em si (vncshim.c) é compilado
# separadamente para cada SO (libvncshim.so no Linux, libvncshim.dll no
# Windows via instalar.ps1) e o resto deste widget (Cairo, GLib.idle_add,
# threading) já é multiplataforma sem alteração nenhuma.
_NOME_SHIM = "libvncshim.dll" if sys.platform == "win32" else "libvncshim.so"
...
def _carregar_shim():
    ...
    for caminho in (os.path.join(aqui, _NOME_SHIM), _NOME_SHIM):
        try:
            ...
            continue
    raise OSError(
        "%s nao encontrada. Compile com instalar.sh/instalar.ps1 — ela "
        "precisa ficar ao lado de vncwidget.py." % _NOME_SHIM)
